...
//
                 ...
                 const typename std::enable_if<(sizeof(ValueType) == 1U)>::type* = nullptr) noexcept
  {
    ...
    const auto result_read_is_ok = mcal_memory_sram_device_type::read(src_addr, &byte_to_read);
                 ...
                 const typename std::enable_if<(sizeof(ValueType) != 1U)>::type* = nullptr) noexcept
  {
    ...
    const auto result_read_is_ok =
      mcal_memory_sram_device_type::read_n(src_addr, reinterpret_cast<std::uint8_t*>(&value_to_read), sizeof(local_value_type));
  ...
  void write(const ValueType src_value,
             ...
             const typename std::enable_if<(sizeof(ValueType) == 1U)>::type* = nullptr) noexcept
  {
    const auto result_write_is_ok = mcal_memory_sram_device_type::write(dest_addr, &src_value);
             ...
             const typename std::enable_if<(sizeof(ValueType) != 1U)>::type* = nullptr) noexcept
  {
    ...
    const auto result_write_is_ok = mcal_memory_sram_device_type::write_n(dest_addr, reinterpret_cast<const std::uint8_t*>(&src_value), sizeof(local_value_type));
